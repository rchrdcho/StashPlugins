(() => {
  ...
  console.log("[ScrapeLens] v1.0.0 loaded");
  ...
  // Use window.__scrapeLensDebug = true to turn on debug mode
  let _debug = false;
  ...
  function log(tag, ...args) {
    if (_debug) console.log(`[ScrapeLens] ${tag}:`, ...args);
  }

  function time(label) {
    if (_debug) console.time(`[ScrapeLens] ${label}`);
  }

  function timeEnd(label) {
    if (_debug) console.timeEnd(`[ScrapeLens] ${label}`);
  }
